...
final kFeedViewType = 'net.goc.oceantide/pangle_expressfeedview';

/// Display feed AD
/// PlatformView does not support Android API level 19 or below.
class ExpressFeedView extends StatefulWidget {
  final String id;
  ...
  ExpressFeedView({
    ...
    this.isUserInteractionEnabled,
        ...
        super(key: key ?? ExpressFeedViewKey(id));

  @override
  State<StatefulWidget> createState() => ExpressFeedViewState();
}

class ExpressFeedViewKey extends GlobalObjectKey {
...
class ExpressFeedViewState extends State<ExpressFeedView>
  ...
  void _onPlatformViewCreated(BuildContext context, int id) {
    ...
    final controller = ExpressFeedViewController._(
      id,
      onRemove: removed,
      onUpdate: updated,
    );
...
class ExpressFeedViewController {
  MethodChannel _methodChannel;
  final VoidCallback onRemove;
  final SizeCallback onUpdate;

  ExpressFeedViewController._(
    int id, {
    this.onRemove,
    this.onUpdate,
  }) {
